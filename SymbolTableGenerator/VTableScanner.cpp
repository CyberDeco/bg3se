...
public:
    struct ExtractionResult {
        std::vector<EssentialSymbol> found_symbols;
        std::vector<std::string> found_lua_bridges;
        std::vector<std::string> missing_symbols;
        int total_symbols_scanned = 0;
    };
    
    ExtractionResult extractTargetedSymbols(const std::string& binary_path) {
        ...
        extractSymbolTable(file, header.ncmds, result);
        
        // Find essential symbols
        findEssentialSymbols(result);
        
        // Find Lua bridge functions
        findLuaBridgeFunctions(result);
        
        return result;
    }
    
    void exportToHeaderFile(const ExtractionResult& result, const std::string& output_path) {
        ...
        out << "// Generated from targeted vtable scan\n\n";
        out << "#include <cstdint>\n\n";
        ...
        for (const auto& symbol : result.found_symbols) {
            if (symbol.critical) {
                std::string safe_name = makeSafeName(symbol.name);
                out << "    constexpr uintptr_t " << safe_name << " = 0x" 
                    << std::hex << symbol.address << "; // " << symbol.name << "\n";
            }
        }
        out << "}\n\n";
        ...
        if (!result.found_lua_bridges.empty()) {
            out << "// LUA BRIDGE FUNCTIONS\n";
            out << "namespace lua_bridge {\n";
            out << "    // Note: These are pattern matches, actual addresses may vary\n";
            for (const auto& bridge : result.found_lua_bridges) {
                if (reverse_lookup.count(bridge)) {
                    std::string safe_name = makeSafeName(bridge);
                    out << "    constexpr uintptr_t " << safe_name << " = 0x" 
                        << std::hex << reverse_lookup[bridge] << "; // " << bridge << "\n";
                }
            }
            out << "}\n\n";
        }
        ...
        std::cout << "✅ Exported " << result.found_symbols.size() << " symbols to " << output_path << std::endl;
    }
    ...
    void exportToBinaryMappings(const ExtractionResult& result, const std::string& output_path) {
        ...
        // Add specific mappings from your bg3se_symbol_finder_out.txt + bg3se patterns
        out << "    <!-- Critical Core Initialization (like Windows bg3se) -->\n";
        out << "    <Mapping Name=\"App__UpdatePaths\" Critical=\"true\">\n";
        out << "      <Target Type=\"Absolute\" Offset=\"0x0\" Symbol=\"App__UpdatePaths\" />\n";
        out << "    </Mapping>\n\n";
        
        out << "    <Mapping Name=\"App__LoadGraphicSettings\" Critical=\"true\">\n";
